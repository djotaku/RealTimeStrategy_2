...
public class Minimap : MonoBehaviour, IPointerDownHandler, IDragHandler
{
    ...
    [SerializeField] private float mapScale = 20f; // assuems minimap is a square
    ...
    private void Update()
    {
        if(playerCameraTransform != null) { return; } // just until we make the lobby
    ...
    private void MoveCamera()
    {
        ...
        Vector2 lerp = new Vector2((localPoint.x - minimapRect.rect.x) / minimapRect.rect.width,
            (localPoint.y - minimapRect.rect.y) / minimapRect.rect.height);  // the division is so that it doesn't matter how large our minimap is
